import React from "react";
import { Spotlight } from "./ui/Spotlight";
import { TextGenerateEffect } from "./ui/TextGenerateEffect";
import BorderMagicButton from "./ui/BorderMagicButton";
import { FaLocationArrow } from "react-icons/fa6";

type Props = {};

const examples = [
  "Crafting Seamless Responsive Web Solutions",
  "Transforming Designs into Interactive Experiences",
  "Prototyping with Precision and Speed",
  "Delivering Robust User-Centric Applications",
  "Enhancing Web Interfaces with Expertise",

  "Elevating Interfaces with Dynamic Functionality",
  "Infusing Creativity into Digital Spaces",
  "Empowering Interaction through Thoughtful Design",
  "Crafting Elegance in Every Pixel",
  "Engineering Engaging Web User Journeys",
  "Bridging Design and Functional Brilliance",
  "Sculpting Intuitive, Modern Web Experiences",
  "Harmonizing Aesthetics with Seamless Performance",
  "Transforming Ideas into Digital Reality",
  "Building Future-Ready Web Interfaces",

  "Crafting Legendary Digital User Journeys",
];

const Hero = (props: Props) => {
  return (
    <div className="pb-20 pt-36">
      <Spotlight
        className="-top-40 -left-10 md:-left-32 md:-top-20 h-screen "
        fill="#653cff"
      />
      <Spotlight className="top-28 left-80  h-[80vh] w-[50vw]" fill="#8f71ff" />
      <Spotlight
        className="top-40 left-full h-[80vh] w-[50vw] "
        fill="#13427e"
      />

      <div className="h-screen w-full dark:bg-black-100 bg-white  dark:bg-grid-white/[0.05] bg-grid-black/[0.2]  flex items-center justify-center absolute top-0 left-0 ">
        <div className="absolute pointer-events-none inset-0 flex items-center justify-center dark:bg-black-100 bg-white [mask-image:radial-gradient(ellipse_at_center,transparent_20%,black)]"></div>
      </div>

      <div className="flex justify-center relative my-20 z-10">
        <div className="max-w-[89vw] md:max-w-2xl lg:max-w-[60vw] flex flex-col items-center justify-center ">
          <h2 className="uppercase tracking-widest text-xs text-center text-blue-100 max-w-80">
            Designing Modern Apps with Next.js
          </h2>

          <TextGenerateEffect
            className="text-center text-[40px] md:text-5xl lg:text-6xl "
            words="Crafting Seamless Responsive Web Solutions"
          />

          <p className="text-center md:tracking-wider mb-4 text-sm md:text-lg lg:text-2xl ">
            Hi, I&apos;m Jeevan, I build and maintain web interfaces with
            seamless collaboration.
          </p>

          <a href="#projects">
            <BorderMagicButton
              title="Explore My Projects."
              icon={<FaLocationArrow />}
              position="RIGHT"
            />
          </a>
        </div>
      </div>
    </div>
  );
};

export default Hero;
